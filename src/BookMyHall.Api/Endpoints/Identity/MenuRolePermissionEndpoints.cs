        ...
        this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet(
                ...
                CancellationToken cancellationToken) =>
            {
                var response =
                    await mediator.Send(
                        new GetMenuRolePermissionsByRoleIdQuery(
                            roleId),
                        cancellationToken);
            ...
            .WithDescription(
                "Returns all menu permissions assigned to a role.")
            .Produces<
        ...
        group.MapPost(
            "/upsert",
            async (
                UpsertMenuRolePermissionCommand command,
                IMediator mediator,
                CancellationToken cancellationToken) =>
            {
                var response =
            ...
            .WithDescription(
                "Creates or updates menu permissions for a role.")
            .Produces<
                ApiResponse<IReadOnlyList<MenuRolePermissionDto>>>(
                StatusCodes.Status200OK)
