...
public sealed class HallRepository(BookMyHallDbContext context) : IHallRepository
{
    ...
    public async Task<PaginatedResult<HallListView>> GetAllAsync(PaginationRequest request,Guid? hallOwnerId = null,CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.SearchText))
        {
            ...
            query = query.Where(x => EF.Functions.ILike(x.HallName,searchPattern)
                || ( x.Description != null && EF.Functions.ILike(x.Description,searchPattern))
                || (x.HallOwnerName != null && EF.Functions.ILike(x.HallOwnerName,searchPattern))
                || (x.HallCategoryName != null && EF.Functions.ILike(x.HallCategoryName,searchPattern))
                || (x.CancellationPolicyName != null && EF.Functions.ILike(x.CancellationPolicyName,searchPattern))
                || (x.AreaName != null && EF.Functions.ILike(x.AreaName, searchPattern))
                || (x.CityName != null && EF.Functions.ILike(x.CityName,searchPattern))
                || (x.DistrictName != null &&EF.Functions.ILike(x.DistrictName,searchPattern))
                || (x.StateName != null &&EF.Functions.ILike(x.StateName,searchPattern))
                || (x.CountryName != null && EF.Functions.ILike(x.CountryName,searchPattern))
                || (x.AddressLine1 != null &&EF.Functions.ILike(x.AddressLine1,searchPattern))
                || ( x.AddressLine2 != null &&EF.Functions.ILike( x.AddressLine2,searchPattern))
                || (x.Pincode != null && EF.Functions.ILike(x.Pincode,searchPattern))
                ...
                || (x.ApprovalStatus != null && EF.Functions.ILike(x.ApprovalStatus,searchPattern))
                || (x.VerificationStatus != null &&EF.Functions.ILike(x.VerificationStatus,searchPattern))
            );
        }
        // =============================================================
        // Total Records
    ...
    {
        var query = context.Halls.AsNoTracking().Where(x => !x.IsDeleted && x.IsActive);

        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            var search = searchTerm.Trim();
            var pattern = $"%{search}%";
            query = query.Where(x => EF.Functions.ILike(x.HallName, pattern));
        }
        ...
        return await query
            ...
            .Select(x => new AutoCompleteItem(x.HallId, x.HallName))
            .ToListAsync(cancellationToken);
    }
}