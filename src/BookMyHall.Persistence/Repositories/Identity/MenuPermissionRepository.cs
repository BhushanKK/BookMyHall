...
public sealed class MenuPermissionRepository(BookMyHallDbContext context): IMenuPermissionRepository
{
    public async Task<MenuPermission?> GetByIdAsync(Guid menuPermissionId,CancellationToken cancellationToken = default)
        => await context.MenuPermissions
            .FirstOrDefaultAsync(x => x.MenuPermissionId == menuPermissionId,cancellationToken);

    public async Task<PaginatedResult<MenuPermission>> GetAllAsync(PaginationRequest paginationRequest,CancellationToken cancellationToken = default)
    {
        var query = context.MenuPermissions.AsNoTracking();
    ...
    public async Task<IReadOnlyList<MenuPermission>> GetByMenuIdAsync( Guid menuId,CancellationToken cancellationToken = default)
        => await context.MenuPermissions
