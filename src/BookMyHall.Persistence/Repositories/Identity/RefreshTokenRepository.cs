...
public sealed class RefreshTokenRepository(BookMyHallDbContext context): IRefreshTokenRepository
{
    ...
    public async Task<RefreshTokenWithUserDto?> GetByTokenAsync(string token,CancellationToken cancellationToken = default)
    {
    ...
    public async Task RevokeAllByUserIdAsync(Guid userId,CancellationToken cancellationToken = default)
    {
        ...
        var refreshTokens = await context.RefreshTokens.AsNoTracking()
    ...
    public async Task<bool> TryRevokeAsync(Guid refreshTokenId,Guid revokedBy,CancellationToken cancellationToken = default)
    {
        var now =DateTimeOffset.UtcNow;
        var affectedRows = await context.RefreshTokens.AsNoTracking()
            .Where(x =>x.RefreshTokenId == refreshTokenId &&!x.IsRevoked && x.ExpiresAt > now)
            .ExecuteUpdateAsync(setters => setters
                    .SetProperty(x => x.IsRevoked,true)
                    .SetProperty( x => x.RevokedAt,now)
                    .SetProperty(x => x.RevokedBy,revokedBy),cancellationToken);
