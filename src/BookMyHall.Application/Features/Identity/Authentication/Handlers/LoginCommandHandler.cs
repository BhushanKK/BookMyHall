    ...
    : IRequestHandler<LoginCommand, ApiResponse<LoginResponse>>
{
    public async Task<ApiResponse<LoginResponse>> Handle(LoginCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!validationResult.IsValid)
            return ApiResponse<LoginResponse>.FailureResponse(string.Join(" | ", validationResult.Errors.Select(x => x.ErrorMessage)), HttpStatusCode.BadRequest);
        ...
        var userSession = new UserSession
        {
            ...
            IpAddress = clientInfoService.IpAddress,
            UserAgent = clientInfoService.UserAgent,
            SessionStart = now,
            LastActivity = now,
            IsActive = true,
            CreatedBy = user.UserId
        ...
        await outboxService.AddAsync(loginEvent, cancellationToken);
    ...
    private async Task<Device> GetOrCreateDeviceAsync(Guid userId, LoginCommand request, DateTimeOffset now, CancellationToken cancellationToken)
    {
