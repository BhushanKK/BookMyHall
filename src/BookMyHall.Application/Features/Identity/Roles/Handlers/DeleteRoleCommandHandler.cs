...
public sealed class DeleteRoleCommandHandler(IRoleRepository roleRepository,IUnitOfWork unitOfWork,
    ...
    : IRequestHandler<DeleteRoleCommand, ApiResponse<bool>>
{
    public async Task<ApiResponse<bool>> Handle(DeleteRoleCommand request,CancellationToken cancellationToken)
    {
        ...
        if (role is null)
        {
            ...
            (ResourceNames.Entities,EntityKeys.Role),HttpStatusCode.NotFound);
        }
