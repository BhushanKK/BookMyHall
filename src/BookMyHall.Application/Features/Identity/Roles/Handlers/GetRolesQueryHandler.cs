...
public sealed class GetRolesQueryHandler(IRoleRepository roleRepository,IMapper mapper,
    ...
    : IRequestHandler<GetRolesQuery, ApiResponse<PaginatedResponse<Role>>>
{
    public async Task<ApiResponse<PaginatedResponse<Role>>> Handle(GetRolesQuery request,CancellationToken cancellationToken)
    {
        ...
        {
            return ApiResponse<PaginatedResponse<Role>>.SuccessResponse(cachedResponse,
        ...
        return ApiResponse<PaginatedResponse<Role>>.SuccessResponse(response,
        messageHelper.RetrievedEntity(ResourceNames.Entities, EntityKeys.Role), HttpStatusCode.OK);
    }
}