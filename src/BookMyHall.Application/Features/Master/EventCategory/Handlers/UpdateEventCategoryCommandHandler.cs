    ...
    : IRequestHandler<UpdateEventCategoryCommand, ApiResponse<EventCategoryDto>>
{
    public async Task<ApiResponse<EventCategoryDto>> Handle(UpdateEventCategoryCommand request,CancellationToken cancellationToken)
    {
        ...
        {
            return ApiResponse<EventCategoryDto>.FailureResponse(messageHelper.NotFound(EntityKeys.EventCategory),HttpStatusCode.NotFound);
        }
        ...
        if (existingEventCategory is not null &&
            existingEventCategory.EventCategoryId != request.EventCategoryId)
        {
