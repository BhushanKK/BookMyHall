...
public sealed class DeleteAmenityCommandHandler(IAmenityRepository amenityRepository,
...
{
    public async Task<ApiResponse<bool>> Handle(DeleteAmenityCommand request,CancellationToken cancellationToken)
    {
        var amenity = await amenityRepository.GetByIdAsync(request.AmenityId, cancellationToken);

        if (amenity is null)
        {
            return ApiResponse<bool>.FailureResponse( messageHelper.NotFound(EntityKeys.Amenity), HttpStatusCode.NotFound);
        }
