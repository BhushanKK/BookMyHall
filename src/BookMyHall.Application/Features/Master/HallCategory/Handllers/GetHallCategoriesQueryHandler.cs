...
public sealed class GetHallCategoriesQueryHandler(IHallCategoryRepository hallCategoryRepository,
    ...
    : IRequestHandler<GetHallCategoriesQuery, ApiResponse<PaginatedResponse<HallCategory>>>
{
    public async Task<ApiResponse<PaginatedResponse<HallCategory>>> Handle(GetHallCategoriesQuery request, CancellationToken cancellationToken)
    {
        ...
        var cachedResponse = await cacheService.GetAsync<PaginatedResponse<HallCategory>>(cacheKey, cancellationToken);

        if (cachedResponse is not null)
        {
            return ApiResponse<PaginatedResponse<HallCategory>>.SuccessResponse
            (
                cachedResponse,
        ...
        var response = new PaginatedResponse<HallCategory>
        {
        ...
        return ApiResponse<PaginatedResponse<HallCategory>>.SuccessResponse(response,
