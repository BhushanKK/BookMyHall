...
public sealed class UpdateCountryCommandHandler(ICountryRepository countryRepository,
    IUnitOfWork unitOfWork,IMapper mapper,IValidator<UpdateCountryCommand> validator,
    IMessageHelper messageHelper,ICacheService cacheService)
    : IRequestHandler<UpdateCountryCommand, ApiResponse<CountryDto>>
{
    public async Task<ApiResponse<CountryDto>> Handle(UpdateCountryCommand request,CancellationToken cancellationToken)
    {
        ...
        if (country is null)
        {
            return ApiResponse<CountryDto>.FailureResponse(messageHelper.NotFound(EntityKeys.Country),HttpStatusCode.NotFound);
        }
