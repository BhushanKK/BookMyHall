...
public sealed class CreateHallBlockCommandHandler(IHallBlockRepository hallBlockRepository,
    IUnitOfWork unitOfWork,IMapper mapper,IValidator<CreateHallBlockCommand> validator,
    IMessageHelper messageHelper,ICacheService cacheService)
    : IRequestHandler<CreateHallBlockCommand,ApiResponse<HallBlockDto>>
{
    public async Task<ApiResponse<HallBlockDto>> Handle(CreateHallBlockCommand request,CancellationToken cancellationToken)
    {
        ...
        var response =mapper.Map<HallBlockDto>(hallBlock);
        return ApiResponse<HallBlockDto>.SuccessResponse(response,messageHelper.AddedEntity(
                ResourceNames.Entities,EntityKeys.HallBlock),HttpStatusCode.Created);
    }
}