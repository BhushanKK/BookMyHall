...
public sealed class UpdateHallBlockCommandHandler(IHallBlockRepository hallBlockRepository,IUnitOfWork unitOfWork,
    IMapper mapper,IValidator<UpdateHallBlockCommand> validator,IMessageHelper messageHelper,
    ...
    : IRequestHandler<UpdateHallBlockCommand,ApiResponse<HallBlockDto>>
{
    public async Task<ApiResponse<HallBlockDto>> Handle(UpdateHallBlockCommand request,CancellationToken cancellationToken)
    {
        ...
        return ApiResponse<HallBlockDto>.SuccessResponse(response,messageHelper.UpdatedEntity(
