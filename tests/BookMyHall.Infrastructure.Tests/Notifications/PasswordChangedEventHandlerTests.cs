...
public sealed class PasswordChangedEventHandlerTests : IDisposable
{
    ...
    public PasswordChangedEventHandlerTests()
    {
        _webRootPath = Path.Combine(
            Path.GetTempPath(),
            $"PasswordChangedEventHandlerTests_{Guid.NewGuid():N}");
    ...
    public async Task Handle_ShouldRenderTemplateAndSendEmail()
    {
        ...
        const string renderedHtml =
            "<html><body>Your password has been changed.</body></html>";
    ...
    {
        // Arrange
        const string renderedHtml ="<p>Your password has been changed.</p>";
        ...
        Assert.Equal("Your BookMyHall password has been changed",email.Subject);
    ...
    public async Task Handle_ShouldAddLogoAsInlineAttachment()
    {
        ...
        _templateServiceMock
            .Setup(x => x.RenderAsync(
                "PasswordChanged",It.IsAny<Dictionary<string, string>>(),
                It.IsAny<CancellationToken>())).ReturnsAsync("<p>Password changed.</p>");
    ...
    public async Task Handle_WhenLogoDoesNotExist_ShouldThrowFileNotFoundException()
    {
        ...
        environmentMock.SetupGet(x => x.WebRootPath).Returns(missingWebRoot);

        var sut = new PasswordChangedEventHandler(
    ...
    public async Task Handle_ShouldPassCancellationTokenToDependencies()
    {
        ...
        _templateServiceMock
            .Setup(x => x.RenderAsync("PasswordChanged",
                It.IsAny<Dictionary<string, string>>(),cancellationToken))
            .ReturnsAsync("<p>Password changed.</p>");
        ...
        // Assert
        _templateServiceMock.Verify(
            x => x.RenderAsync("PasswordChanged",It.IsAny<Dictionary<string, string>>(),
                cancellationToken),Times.Once);

        _emailSenderMock.Verify(x => x.SendAsync(It.IsAny<EmailMessage>(),
                cancellationToken),Times.Once);
    }

    private PasswordChangedEventHandler CreateSut()
    {
        return new PasswordChangedEventHandler(
            _templateServiceMock.Object,
            _emailSenderMock.Object,
            _environmentMock.Object);
    }

    private static PasswordChangedEvent CreateNotification()
    {
        return new PasswordChangedEvent(Guid.NewGuid(),UserName,Email);
    }
